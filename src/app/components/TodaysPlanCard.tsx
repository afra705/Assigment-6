    ...
    <div className="grid grid-cols-1">
      {sortedMyPlan.length > 0 ? (
        sortedMyPlan.map((item) => (
          <div
            key={item.id}
            className="bg-[#15181f] border border-gray-800 rounded-xl p-3 mt-4 w-full"
          >
            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
              ...
              <div className="shrink-0">
                <Image
                  ...
                  height={70}
                  alt={item.name}
                  className="w-full sm:w-30 h-40 sm:h-20 object-cover rounded-lg"
                />
              </div>
              ...
              <div className="flex flex-wrap sm:flex-nowrap items-center gap-2 shrink-0">
                <Link href={`/workouts/${item.id}`}>
                  <button className="border border-gray-700 text-gray-300 rounded-full px-3 sm:px-4 py-2 text-xs hover:bg-gray-800">
                    View Details
                  </button>
                </Link>
                ...
                <button
                  ...
                  className="bg-[#c2f800] text-black font-bold rounded-full px-3 sm:px-4 py-2 text-xs"
                >
        ...
        <div className="p-8 sm:p-16 md:p-40 border text-center border-gray-800">
          <h2 className="font-bold text-2xl sm:text-3xl text-white">
            NOTHING HERE YET
          </h2>

          <p className="text-gray-500 mt-2 mb-2 text-sm">
            Browse the library and add a lift to get today moving.
          </p>

          <Link href="/workouts">
            <button className="bg text-black p-3 rounded-2xl font-bold text-[12px]">
              Go to workouts
            </button>
          </Link>
        </div>
